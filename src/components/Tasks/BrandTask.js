...
class BrandTask extends React.Component {
    ...
    handleKeyDownEvent = (event) => {
        if (event.keyCode === SPACE_KEY_CODE) {
            const { selectedOption, selectionCompleted } = this.state
    ...
    validateInput = (brand) => {
        ...
        this.setState({ selectedOption: selectedOption, selectionCompleted: selectedOption.length > 0 })
    }
    ...
    render() {
        ...
        return (
            <Container className="justify-content-md-center">
                ...
                {this.state.selectionCompleted ? <div style={{ 'marginTop': '25px' }}><Footer text={TEXT_FOOTER} /></div> : <></>}
            </Container>
        );
    }
}
